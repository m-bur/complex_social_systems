...
def generate_parameters():
    ...
    target_opinion = np.linspace(-1, 0.9, num=20)
    average_factor = 20
    for m_shift, n_media in zip(media_shift, number_of_media):
        for t_opinion in target_opinion:
            for i in range(average_factor):
                ...
                parameters.append(["--number_of_manipulated_media", str(n_media), "--manipulation_shift", str(
                    m_shift), "--target_media_opinion", str(t_opinion), "--parent_folder", name, "--number_years", "5"])

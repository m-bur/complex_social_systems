...
def calibrate_parameters(args=None):
    for i in range(3, 4):
        ...
        for _, sigma in enumerate(param_ranges["sigma"]):
            ...
            final_NV, final_std, final_clustering, final_pol, prob_to_change = (
                run_simulation(args)
            )
...
def plot_calibration_heatmap():
    ...
    pivot_data_std = df.pivot(
        index="media_authority", columns="initial_threshold", values="final_std"
    )
    ...
    plots = [
        pivot_data_opinion,
        pivot_data_std,
        pivot_data_clustering,
        pivot_data_prob_to_change,
    ]
    titles = [
        "Non-Voters",
        "Standard Deviation",
        "Clustering",
        "Opinion Changes per Year per Voter",
    ]
    ...
    for idx, (ax, data, title, key) in enumerate(
        zip(axes, plots, titles, norms.keys())
    ):
        cax = ax.imshow(
            data, cmap="seismic", norm=norms[key], origin="lower", aspect="equal"
        )
        ax.set_title(title)
...
def plot_calibration_init_threshold():
    ...
    output_path = (
        "calibrations/initial_threshold_calibration_results/calibration_result_3.pdf"
    )
    ...
    df["initial_threshold_1"] = (
        df["initial_threshold_1"].str.replace("(", "").str.replace(")", "")
    )
    df["initial_threshold_2"] = (
        df["initial_threshold_2"].str.replace("(", "").str.replace(")", "")
    )
    ...
    # Pivot data for imshow
    pivot_data_opinion = df.pivot(
        index="initial_threshold_1",
        columns="initial_threshold_2",
        values="final_opinion",
    )
    pivot_data_std = df.pivot(
...
def plot_calibration_media_feedback():
    ...
    aggregated_data = (
        combined_data.groupby("media_feedback_probability")
        .agg(
...
def plot_calibration_media_number():
    ...
    # Compute the average and standard deviation for each threshold parameter
    aggregated_data = (
        combined_data.groupby("number_media")
        .agg(
            final_opinion_mean=("final_opinion", "mean"),
            final_opinion_std=("final_opinion", "std"),
            final_std_mean=("final_std", "mean"),
...
def plot_calibration_alpha():
    ...
    # Compute the average and standard deviation for each threshold parameter
    aggregated_data = (
        combined_data.groupby("threshold_parameter")
        .agg(
            final_opinion_mean=("final_opinion", "mean"),
            final_opinion_std=("final_opinion", "std"),
            final_std_mean=("final_std", "mean"),
...
def plot_extremism():
    ...
    # Compute the average and standard deviation for each threshold parameter
    aggregated_data = (
        combined_data.groupby("std_media_opinion")
        .agg(
            final_opinion_mean=("final_opinion", "mean"),
            final_opinion_std=("final_opinion", "std"),
            final_std_mean=("final_std", "mean"),

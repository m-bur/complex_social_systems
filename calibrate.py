...
def parse_args():
    ...
    parser.add_argument("--media_feedback_turned_on_after",
                        type=int, default=10*365)
    parser.add_argument("--media_feedback_probability",
                        type=float, default=0.1)
    parser.add_argument(
        "--media_feedback_threshold_replacement_neutral", type=float, default=0.1)
    parser.add_argument("--number_of_days_election_cycle",
                        type=int, default=50)
...
def run_simulation(args):
    ...
    if regen_network:
        ...
        df_conx = pd.read_csv(network_path, converters={
                              "connection": literal_eval})

    folder = "gaussian"
    print_parameters(args, folder, "parameters.txt")
    network = init_network(df_conx, L, mfeedback_prob,
                           mfeedback_threshold_replacement)  # LxL network of voters
    ...
    for days in range(Ndays):
        if days >= 365:
            ...
            media = update_media(days, media, election_results, mu,
                                 number_of_days_election_cycle, x, y, manipulation_shift=manipulation_shift)
        # turn media feedback on
        if days == mfeedback_after:
            mfeedback = mfeedback_on
        changed_voters += network_update(network,
                                         media, Nv, w, t0, alpha, mfeedback)
        ...
        if days == manipulation_day:
            turn_on_media_manipulation_by_opinion_distance(
                media=media, N=number_of_manipulated_media, target_opinion=target_media_opinion)
...
def calibrate_parameters(args=None):
    for i in range(3, 4):
        ...
        for _, sigma in enumerate(param_ranges["sigma"]):
            ...
            final_NV, final_std, final_clustering, final_pol, prob_to_change = run_simulation(
                args)
        ...
        with open(summary_log, "w") as f:
            f.write(
                "std_media_opinion,final_opinion,final_std,final_clustering,final_pol,prob_to_change\n")
            for log in logs:
                f.write(
...
def plot_calibration_heatmap():
    ...
    df['media_authority'] = pd.to_numeric(
        df['media_authority'], errors='coerce')
    df['initial_threshold'] = pd.to_numeric(
        df['initial_threshold'], errors='coerce')
    df['final_opinion'] = pd.to_numeric(df['final_opinion'], errors='coerce')
    df['final_std'] = pd.to_numeric(df['final_std'], errors='coerce')
    df['final_clustering'] = pd.to_numeric(
        df['final_clustering'], errors='coerce')
    df['final_polarization'] = pd.to_numeric(
        df['final_polarization'], errors='coerce')
    ...
    pivot_data_opinion = df.pivot(
        index='media_authority', columns='initial_threshold', values='final_opinion')
    pivot_data_std = df.pivot(index='media_authority',
                              columns='initial_threshold', values='final_std')
    ...
    pivot_data_pol = df.pivot(
        index='media_authority', columns='initial_threshold', values='final_polarization')
    ...
    plots = [pivot_data_opinion, pivot_data_std,
             pivot_data_clustering, pivot_data_prob_to_change]
    titles = ['Non-Voters', 'Standard Deviation',
              'Clustering', 'Opinion Changes per Year per Voter']

    # fonts
    plt.rc('text', usetex=True)
    plt.rc('font', family='sans-serif')  # Use a serif font for LaTeX style
    # Use sans-serif math mode
    ...
    for idx, (ax, data, title, key) in enumerate(zip(axes, plots, titles, norms.keys())):
        cax = ax.imshow(data, cmap='seismic',
                        norm=norms[key],  origin='lower', aspect='equal')
        ax.set_title(title)
      ...
      # Set ticks to match the pivot table indices and columns
        ...
        ax.set_xticklabels(
            [f"{val:.2f}" for val in x_ticks], rotation=45, fontsize=16)
...
def plot_calibration_init_threshold():
    ...
    df['initial_threshold_1'] = df['initial_threshold_1'].str.replace(
        "(", "").str.replace(")", "")
    df['initial_threshold_2'] = df['initial_threshold_2'].str.replace(
        "(", "").str.replace(")", "")
    df['initial_threshold_1'] = pd.to_numeric(
        df['initial_threshold_1'], errors='coerce')
    df['initial_threshold_2'] = pd.to_numeric(
        df['initial_threshold_2'], errors='coerce')
    df['final_opinion'] = pd.to_numeric(df['final_opinion'], errors='coerce')
    df['final_std'] = pd.to_numeric(df['final_std'], errors='coerce')
    df['final_clustering'] = pd.to_numeric(
        df['final_clustering'], errors='coerce')
    ...
    # Pivot data for imshow
    pivot_data_opinion = df.pivot(
        index='initial_threshold_1', columns='initial_threshold_2', values='final_opinion')
    pivot_data_std = df.pivot(
        index='initial_threshold_1', columns='initial_threshold_2', values='final_std')
    pivot_data_clustering = df.pivot(
        index='initial_threshold_1', columns='initial_threshold_2', values='final_clustering')
    ...
    for idx, (ax, data, title) in enumerate(zip(axes, plots, titles)):
      ...
      # Set ticks to match the pivot table indices and columns
        ax.set_xticks(range(len(x_ticks)))
        ax.set_xticklabels(
            [f"{val:.2f}" for val in x_ticks], rotation=45, fontsize=14)
        ax.set_yticks(range(len(y_ticks)))
...
def plot_calibration_media_feedback():
    ...
    file_paths = glob.glob(
        "calibrations/mfeedback1_calibration_results/calibration_log*.txt")
    ...
    plt.tight_layout()
    plt.savefig(
        "calibrations/mfeedback1_calibration_results/mfeedback_calibration.pdf")
    plt.savefig(
        "calibrations/mfeedback1_calibration_results/mfeedback_calibration.png")


def plot_calibration_media_number():
    # Define file paths and load data from all files
    file_paths = glob.glob(
        "calibrations/nmedia_calibration_results/calibration_log*.txt")
    dataframes = [pd.read_csv(file, sep=",") for file in file_paths]
...
def plot_calibration_alpha():
    # Define file paths and load data from all files
    file_paths = glob.glob(
        "calibrations/alpha_calibration_results_1/calibration_log*.txt")
    dataframes = [pd.read_csv(file, sep=",") for file in file_paths]
